...
$('#form').submit(function () {
        ...
        if ($('#name').val() == '' || $('#email').val() == ''  || $('#message').val() == '' || $('#phone').val() == '')  {
...
/* <!$('#form-second').submit(function () {
	...
	if ($('#name-second').val() == '' || $('#email-second').val() == ''  || $('#message-second').val() == '' || $('#phone-second').val() == '')  {
		alert('יש למלא את כל השדות')
		return false;
	} else {
	if ($('#phone-second').val() == '' || /^[\+]?[(]?[0-9]{2,3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/.test($('#phone-second').val()) == false) {
		alert('נא לוודא מספר טלפון תקין');
		return false;
